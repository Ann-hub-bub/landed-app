...
Response pattern:
1. One dry, sarcastic line that gets straight to the point
...
- If the user wants a human, wants to talk to Anna, or wants to book a call — only then end your reply with this exact HTML button on its own line: <a href="https://calendly.com/aboytsova9/coffee-break" target="_blank" rel="noopener" class="bookCallAnna" style="display:inline-flex;align-items:center;gap:8px;background:#ff6a1f;color:#0d0d0d;font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:14px;padding:12px 22px;text-decoration:none;border-radius:2px;">Book a call with Anna &nbsp;→</a>
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await fetch('https://script.google.com/macros/s/AKfycbyP0O7xn4wW_ii3INRgC60uvZtjPPuyxwOL-5fYIIZ2iu7e_laQ0AiIJyxdTaDdQE7KOg/exec', {
      method: 'POST',
    ...
    }).then(r => { if (!r.ok) console.error('Sheets log failed:', r.status); })
